...
cpu_time = {}
ocl_time = {}
output = subprocess.check_output([EXE_NAME, USI_NAME],
...
COL_NAMES = [
    "method", "CPU time (ms)", "OpenCL GPU time (ms)", "accelerate ratio"
]
    ...
    for method_cpu in cpu_time.keys():
        for method_ocl in ocl_time.keys():
            if method_ocl == method_cpu + '_ocl':
                acc_ratio[
                    method_cpu] = cpu_time[method_cpu] / ocl_time[method_ocl]
                f_csv.writerow([
                    method_cpu,
