...
/* define `DEBUG` before including "ocl_utils.h" to enable DBG_PRINT */
#include "debug.h"
...
static cl_int ocl_get_best_gpus(cl_uint num_entries, cl_device_id *best_gpus, cl_uint *num_best_gpus)
{
    ...
    for(size_t i = 0; i < platform_cnt; ++i)
    {
        ...
        if (status != CL_SUCCESS)
	    {
		    DBG_PRINT("Failed to get the size of platform[%d]'s name.\n", i);
            free(platforms);
		    return status;
	    }
        ...
        if (status != CL_SUCCESS)
	    {
		    DBG_PRINT("Failed to get the name of platform[%d].\n", i);
            free(platform_name);
            free(platforms);
		    return status;
	    }
	...
	if (*status != CL_SUCCESS)
	{
		...
		for(size_t i = 0; i < device_cnt; ++i)
		{
			*status = clGetProgramBuildInfo( program, devices[i], CL_PROGRAM_BUILD_STATUS, sizeof build_status, &build_status, NULL);
			if(*status != CL_SUCCESS)
			{
				DBG_PRINT("\nFailed to get build status of device %d\n", i);
				continue;
			}
			if(build_status == CL_BUILD_NONE)
			{
				DBG_PRINT("\nNo build operations have been performed on the specified program for device %d\n", i);
				continue;
			}
